...
const getFormAndCandidateDataByEmail = async (req, res) => {
    const { email } = req.params;

    try {
        const data = await formModel.getFormAndCandidateDataByEmail(email);
        if (!data) {
            return res.status(404).json({ error: 'No se encontraron datos para este email' });
        }
        res.status(200).json(data);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Error al obtener los datos del formulario y candidato' });
    }
};
